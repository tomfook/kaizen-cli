...
- **knowledge/**（context/ ではなく）: 「知識蓄積型」のブランディングに合わせ、LLM の「コンテキストウィンドウ」との混同を避けるために選択
- **committing-project を除外**: コミットフォーマットは改善サイクルの本質ではない。Claude Code にはビルトインのコミット規約がある
