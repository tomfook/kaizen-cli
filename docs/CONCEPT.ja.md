...
作業を通じて蓄積される知識は、自然と2つのカテゴリに分かれます — そしてこれらを混在させると問題が生じます。

**プロジェクト固有の知識**は、1つのプロジェクトに属するものです。アーキテクチャの決定、技術スタックの選択、命名規則、ステークホルダーの要件は、そのプロジェクトでは重要ですが、他のプロジェクトでは無関係 — あるいは誤解の元になります。

**プロジェクト横断の知識**は、特定のプロジェクトを超えて価値を持ちます。APIの落とし穴、ドメインの専門知識、ツールの癖、コーディングパターン、暗黙のルールは、どこでも役立つものです。一度学んだことは、二度と学び直す必要がないはずです。

| 層 | 配置場所 | 例 |
|----|---------|---|
| **プロジェクト固有** | `CLAUDE.md`, `docs/` | アーキテクチャ、設定、ロードマップ、チーム規約 |
| **プロジェクト横断** | `knowledge/`（共有） | ドメイン専門知識、落とし穴の記録、再利用可能なパターン、暗黙のルール |

Kaizen-CLI はこの2つの層を明確に分離します。

プロジェクト固有の文脈は各プロジェクトの `CLAUDE.md` と `docs/` に留まります。プロジェクト横断の知識は共有 `knowledge/` ディレクトリに蓄積され、すべてのプロジェクトにsymlinkされます。reflect-learning がどのプロジェクトで再利用可能な知見を捉えても、手動コピーや同期なしに、他のすべてのプロジェクトで即座に利用可能になります。一方で、プロジェクト固有の情報が境界を越えて他のプロジェクトの文脈を汚染することはありません。

判断ルールはシンプルです: **プロジェクト名を取り除いても意味が通る知見は、`knowledge/` に属します。**
...
1. **プロジェクト固有の情報**（CLAUDE.md、docs/）と**横断的な知識**（knowledge/）が分離されています — 上述の「知識の2つの層」モデルの通りです
