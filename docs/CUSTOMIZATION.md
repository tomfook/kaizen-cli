...
Kaizen-CLI organizes information into three layers by scope:

| Layer | Location | Scope | Updated by |
|-------|----------|-------|------------|
| **Cross-project knowledge** | `knowledge/` | Shared across all projects in a registry | `/kaizen-reflect-learning` |
| **Project-specific lessons** | `docs/LEARNINGS.md` | Failures, constraints, and pitfalls for this project | `/kaizen-update-docs` |
| **Project documentation** | `CLAUDE.md`, `docs/` | Settings, design decisions, plans | `/kaizen-update-docs` |
...
Lessons in `docs/LEARNINGS.md` are periodically reviewed by `/kaizen-update-docs`: general patterns are **graduated** to `knowledge/`, stale entries (resolved constraints) are removed, and lessons already embedded in the design are retired.
