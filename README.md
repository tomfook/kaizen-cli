...
`setup.sh` creates the shared knowledge directory (`$KAIZEN_KNOWLEDGE_DIR`) and installs the `kaizen-init-project` skill globally. The `knowledge/` in each project is a symlink to a registry within this shared directory — knowledge accumulated in one project is automatically available to all others sharing the same registry. You can create multiple registries (e.g., work and personal) to keep knowledge separated by context. Commands and skills are linked per-project when you run `/kaizen-init-project`.
