...
`setup.sh` は共有知識ディレクトリ（`$KAIZEN_KNOWLEDGE_DIR`）の作成とグローバルコマンドのインストールを行います。各プロジェクトの `knowledge/` はこの共有ディレクトリ内のレジストリへのシンボリックリンクとなり、同じレジストリ内のプロジェクトで蓄積された知識が自動的に共有されます。レジストリを複数作成すれば（例: 職場用と個人用）、文脈ごとに知識を分離できます。
