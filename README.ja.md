...
Claude Code の利用者の多くは「単発のコード生成ツール」として使っています。大規模な長期プロジェクトであれば CLAUDE.md やドキュメントに知識を蓄積できますが、小規模プロジェクトを多数並行する業務では、知識がセッションごと・プロジェクトごとに断絶し、同じ調査・同じ失敗を繰り返してしまいます。
